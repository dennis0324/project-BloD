...
export async function generateStaticParams() {
  const res = await fetch(`http://localhost:3000/api/blogposts?foramt=length&postCount=5`,{
    cache: "no-cache" //this need to be set to no-cace to avoid not updating the data
  })
  if (!res.ok) {
    throw new Error('Failed to fetch data')
    ...
    <div className="">
      {
        data.data.map((element,index) => {
          ...
          return (
            <a key={element.id} className="grid grid-cols-6 gap-x-6 gap-y-3 my-4" href={`post/${element.id}`}>
